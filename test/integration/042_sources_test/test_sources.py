...
class TestSourceFreshness(BaseSourcesTest):
    ...
    def _set_updated_at_to(self, delta):
        timefmt = "'%Y-%m-%d %H:%M:%S'"
        # presto, of course, has its own ideas about time formatting
        if self.adapter_type == 'presto':
            timefmt = "cast(from_iso8601_timestamp('%Y-%m-%dT%H:%M:%S') as timestamp)"
        timestr = (datetime.utcnow() + delta).strftime(timefmt)
    ...
    @use_profile('presto')
    def test_presto_source_freshness(self):
        results = self.run_dbt_with_vars(
            ['source', 'snapshot-freshness'],
            expect_pass=False
        )
